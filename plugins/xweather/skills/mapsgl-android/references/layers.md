...
181 built-in weather layers, generated from the MapsGL Android SDK source at the
`release/1.6.1` tag.

**Every code listed here ships in 1.6.1.** The catalog is generated against that release, so
anything below exists in the JitPack artifact you depend on - there are no development-branch
...
Generated from the MapsGL Android SDK source at `release/1.6.1` (`fc747f88`), cross-checked against
the published KDoc for 1.6.1 at `cdn.aerisapi.com/sdk/android/mapsgl/docs/v1.6.1/`.
